...
export async function cachedFetch(endpoint, params = {}, ttlMs = 300000) {
  // If no API key, try free API first
  if (!API_KEY) {
    console.log("No API key, attempting free API fallback...");
    try {
      const response = await fetch("https://www.football-data.org/api/competitions/PL/matches?limit=10");
      if (response.ok) {
